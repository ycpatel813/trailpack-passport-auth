...
module.exports = class PassportService extends Service {
   ...
   */
  register(userInfos) {
    var password = userInfos.password
  ...
  login(identifier, password) {
    ...
    condition = condition.concat({email: identifier});
    
    return this.app.orm.User.findOne().or(condition)
      .then(user => {
        if (!user) {
          throw new Error('E_USER_NOT_FOUND')
        }
        else {
          return this.app.orm.Passport.findOne({user: user._id})
            .then(passport => {
              ...
              return new Promise((resolve, reject) => {
                ...
                if (password == passport.password) {
                  return resolve(onUserLogged(this.app, user))
                }
                else {
